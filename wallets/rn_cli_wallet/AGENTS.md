...
### Slim iOS simulator (simslim)
With `ios-slim-simulator: 'true'` (the WalletKit workflow's default; dispatch can untick `ios-slim`), the composite slims the fresh E2E simulator with [simslim](https://github.com/MobAI-App/simslim) before its first boot: ~140 background daemons disabled, ~1.4 GB instead of ~5.4 GB idle footprint (measured locally on iOS 26.2). The release is pinned with a sha256; the profile is `.github/actions/walletkit-build-and-maestro/simslim-ci.json` (keeps `icloud` and `web` enabled), and `simslim verify` fails the job on drift. `simslim measure` writes the simulator's memory before/after the tests to the job summary for stock runs too. Use `simslim clone`, not `simctl clone`, for extra devices: `simctl clone` drops the slim overrides. The simulator steps sit below the cache-key marker, so editing them doesn't force a rebuild. Dispatch's `ios-runner` input can pick the free standard `macos-latest` to compare against the billed xlarge runner.
