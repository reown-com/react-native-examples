...
export default function SessionBitcoinSignMessageModal() {
  ...
  const displayMessage = request.params?.message || '';
  ...
  const onApprove = useCallback(async () => {
    if (requestEvent) {
      ...
      try {
        ...
        // when signing fails. Surface it so the wallet doesn't look successful.
        const errorMessage =
    ...
    if (requestEvent) {
      ...
      try {
        ...
        handleRedirect({
          peerRedirect: peerMetadata?.redirect,
          isLinkMode: isLinkMode,
          error: 'User rejected Bitcoin message request',
        });
      } catch (e) {
        LogStore.error(
          (e as Error).message,
